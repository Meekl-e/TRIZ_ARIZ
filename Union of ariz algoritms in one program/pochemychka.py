...
def setQuestion(connector):
    ...
    if connector.typeO=="no" and( m !="" or v!="" or w !=""):
        ...
        n = "нет"
    return f"Почему {n} {v} {w}? х для конца ".replace("  "," ")
...
def pochemy(morph, question ):
    ...
    for word in question:
        ...
        if (t.POS == "NOUN" and t.case in ["acc2", "accs", "gent"]) or t.POS == "INFN":
            ...
            verb = p
    listConnectors.append(Connector(typeObject="not", word=w, verb=verb, mainWord=wordMain))
    questions = True
    while questions:
    ...
    for a in listAnswers.keys():
        ...
        for i in range(index - 1, 0, -1):
            answ += setQuestionToAnswer(listConnectors[i]) + "=>"
        allAnswers.append(answ)
    return allAnswers
